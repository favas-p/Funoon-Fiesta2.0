  ...
  return (
    <ChartContext.Provider value={{ config }}>
      <div
        ...
        data-chart={chartId}
        className={cn(
          "relative flex w-full min-w-0 h-[260px] md:h-[280px] justify-center text-xs",
...
function getPayloadConfigFromPayload(
  ...
  const maybeAnyPayload = payload as Record<string, unknown>;
  const payloadPayload =
    "payload" in maybeAnyPayload &&
    typeof maybeAnyPayload.payload === "object" &&
    maybeAnyPayload.payload !== null
      ? (maybeAnyPayload.payload as Record<string, unknown>)
      : undefined;

  let configLabelKey: string = key;

  if (
    key in maybeAnyPayload &&
    typeof maybeAnyPayload[key] === "string"
  ) {
    configLabelKey = maybeAnyPayload[key] as string;
  } else if (
    payloadPayload &&
